...
pub(crate) fn compress_mb16(
    ...
    unsafe {
        let mut aux = [
            _mm256_set1_epi32(IV[0] as _),
            _mm256_set1_epi32(IV[1] as _),
            _mm256_set1_epi32(IV[2] as _),
            _mm256_set1_epi32(IV[3] as _),
            _mm256_set1_epi32(t as _),
            _mm256_setzero_si256(),
            _mm256_set1_epi32(len as _),
            _mm256_set1_epi32(flags as _),
        ];
        ...
        for i in 0..8 {
            v[i] = _mm256_xor_si256(v[i], aux[i]);
        }
    }
}
...
mod tests {
    ...
    fn test_g_function() {
        ...
        g4(
            ...
            vc,
            vd,
            unsafe { _mm256_set1_epi32(crate::sha256::IV[0] as _) },
            unsafe { _mm256_set1_epi32(crate::sha256::IV[1] as _) },
        );

        for i in 0..16 {
            assert_eq!(
                unsafe { _mm_extract_epi32(_mm256_castsi256_si128(state_v[i]), 0) as u32 },
                state[i],
    ...
    fn test_compress_mb16() {
        let mut v =
            core::array::from_fn(|i| unsafe { _mm256_set1_epi32(crate::blake3::IV[i] as _) });
        let mut block = core::array::from_fn(|i| unsafe {
            if i == 0 {
                _mm256_set1_epi32(0x46544549u32 as _)
            } else {
                _mm256_setzero_si256()
            }
        });
        ...
        for i in 0..8 {
            unsafe {
                results[i] = _mm_extract_epi32(_mm256_castsi256_si128(v[i]), 0) as u32;
            }
        }
        assert_eq!(
            results, expected,
    ...
    fn test_compress_mb16_reduced() {
        ...
        unsafe {
            let mut v = core::array::from_fn(|i| _mm256_set1_epi32(prepared_state[i] as _));
            let patch = _mm256_setr_epi32(1, 2, 3, 4, 5, 6, 7, 8);
            compress_mb16_reduced::<4, 15>(&mut v, &block_template, patch);

            let mut extract_h0 = [0u32; 8];
            _mm256_storeu_si256(extract_h0.as_mut_ptr().cast(), v[0]);
            for validate_val in 0..8 {
    ...
    fn test_compress_mb16_256() {
        ...
        for i in 0..8 {
            unsafe {
                output[i] = _mm_extract_epi32(_mm256_castsi256_si128(v[i]), 0) as u32;
            }
            expected[i] = u32::from_le_bytes(hash[i * 4..i * 4 + 4].try_into().unwrap());
        }
