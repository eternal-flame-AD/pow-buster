...
pub fn bench_proof_go_to_social(c: &mut Criterion) {
    ...
    #[cfg(target_feature = "avx512f")]
    {
        ...
        let mut hasher = std::hash::RandomState::default().build_hasher();

        let mut jitter = core::iter::repeat(i16::MIN..=i16::MAX).flatten();
        hasher.write_u64(0);
        let mut seed1 = hasher.finish();
        hasher.write_u64(1);
        let mut seed2 = hasher.finish();
            ...
            b.iter_batched(
                || {
                    ...
                    let answer = core::hint::black_box(
                        (center as i64 + jitter.next().unwrap() as i64) as u64,
                    );
...
pub fn bench_proof_rayon(c: &mut Criterion) {
    ...
    group.throughput(Throughput::Elements(
        (max_nonce - center) * BATCH_SIZE as u64,
    ));
    ...
    group.bench_function("proof_go_to_social_rayon", |b| {
        b.iter_batched(
            || {
            ...
            |data: [_; BATCH_SIZE]| {
                ...
                data.into_par_iter()
                    .for_each(|(seed_bytes, image, answer)| {
                        use pow_buster::{
                            message::GoToSocialMessage,
                            solver::{Solver, avx512::GoToSocialSolver},
                        };
